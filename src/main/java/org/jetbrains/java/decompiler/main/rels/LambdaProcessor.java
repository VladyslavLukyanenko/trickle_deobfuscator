...
public class LambdaProcessor {
  ...
  public LambdaProcessor(IIdentifierRenamer renamer) {
          ...
          for (int i = 0; i < len; ++i) {
            ...
            if (instr.opcode == CodeConstants.opc_invokedynamic) {
              ...
              if (lambdaMethods.get(invoke_dynamic.index1)) { // lambda invocation found
                ...
                String className = content_method_handle.classname;
                final String simpleClassName = className.substring(className.lastIndexOf('/') + 1);
                if (renamer != null && renamer.toBeRenamed(IIdentifierRenamer.Type.ELEMENT_CLASS, simpleClassName, "", "")) {
                  className =className.substring(0, className.lastIndexOf('/') + 1) + renamer.getNextClassName(className, simpleClassName);
                }

                String methodName = content_method_handle.elementname;
                  final String descriptor = content_method_handle.descriptor;
                if (renamer != null && renamer.toBeRenamed(IIdentifierRenamer.Type.ELEMENT_METHOD, className, methodName, descriptor)) {
                  methodName = renamer.getNextMethodName(content_method_handle.classname, methodName, descriptor);
                }

                ClassNode node_lambda = new ClassNode(className, methodName, descriptor, content_method_handle.index1,
