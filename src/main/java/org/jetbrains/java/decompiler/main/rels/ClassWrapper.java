...
public class ClassWrapper {
  ...
  public ClassWrapper(StructClass classStruct, IIdentifierRenamer renamer) {
      ...
      try {
        if (mt.containsCode()) {
          if (maxSec == 0 || testMode) {
            root = MethodProcessorRunnable.codeToJava(classStruct, mt, md, varProc, renamer);
          }
          else {
            MethodProcessorRunnable mtProc = new MethodProcessorRunnable(classStruct, mt, md, varProc, DecompilerContext.getCurrentContext(), renamer);
          ...
          for (int i = 0; i < paramCount; i++) {
            ...
            if (thisVar) {
              if (i == 0) {
                varProc.setVarName(new VarVersionPair(varIndex, 0), null);
                varIndex++;
              }
              else {
                var param = md.params[i - 1];
                varProc.setVarName(new VarVersionPair(varIndex, 0), vc.getFreeName(varIndex, param));
                varIndex += param.stackSize;
              }
            }
            else {
              var param = md.params[i];
              varProc.setVarName(new VarVersionPair(varIndex, 0), vc.getFreeName(varIndex, param));
