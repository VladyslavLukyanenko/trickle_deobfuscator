...
public class ClassesProcessor implements CodeConstants {
  ...
  public ClassesProcessor(StructContext context, IIdentifierRenamer renamer) {
  ...
  private static void initWrappers(ClassNode node, IIdentifierRenamer renamer) {
    if (node.type == ClassNode.CLASS_LAMBDA) {
      return;
    }

    ClassWrapper wrapper = new ClassWrapper(node.classStruct, renamer);
    wrapper.init();

    node.wrapper = wrapper;

    for (ClassNode nd : node.nested) {
      initWrappers(nd, renamer);
    }
  }
  ...
  public static class ClassNode {
    ...
    public ClassNode(String content_class_name,
      ...
      boolean is_method_reference = !(content_class_name.equals(classStruct.qualifiedName));
      if (!is_method_reference) { // content method in the same class, check synthetic flag
        ...
        is_method_reference = mt != null && !mt.isSynthetic(); // if not synthetic -> method reference
      }
