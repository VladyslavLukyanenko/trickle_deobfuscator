...
public class ClassWriter {
  ...
  public void classToJava(ClassNode node, TextBuffer buffer, int indent, BytecodeMappingTracer tracer) {
    ...
    try {
      ...
      final var clsName = DecompilerContext.getImportCollector().getShortName(cl.qualifiedName, true);
      final var fields = cl.getFields();
      fields.sort((f1, f2) -> {
        var field1Descr = FieldDescriptor.parseDescriptor(f1.getDescriptor());
        var field2Descr = FieldDescriptor.parseDescriptor(f2.getDescriptor());
        var f1Cls = ExprProcessor.getTypeName(field1Descr.type, true);
        if (field1Descr.descriptorString.startsWith("[")) {
          f1Cls += "[]";
        }

        var f2Cls = ExprProcessor.getTypeName(field2Descr.type, true);
        if (field2Descr.descriptorString.startsWith("[")) {
          f2Cls += "[]";
        }

        if (clsName == null || clsName.equals(f1Cls) && f1Cls.equals(f2Cls)) {
          return 0;
        }

        if (f1Cls.equals(clsName)) {
          return -1;
        }

        if (f2Cls.equals(clsName)) {
          return 1;
        }

        return f1.getDescriptor().compareTo(f2.getDescriptor());
      });
    ...
    if (cl.getRecordComponents() != null) {
      ...
      if (name.equals("equals") && descriptor.equals("(Ljava/lang/Object;)Z") ||
        ...
        if (code.countLines() == 1) {
          ...
          return str.startsWith("return this." + name + "__INVOKEDYNAMIC__(this");
        }
      }
    }
    return false;
  ...
  private boolean methodToJava(ClassNode node, StructMethod mt, TextBuffer buffer, int indent, BytecodeMappingTracer tracer) {
    ...
    try {
      ...
      // todo: filter InsnComment annotation here
      if (!clInit && !dInit) {
