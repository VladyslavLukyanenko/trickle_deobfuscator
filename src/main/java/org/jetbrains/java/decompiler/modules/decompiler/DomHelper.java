  ...
  private static RootStatement graphToStatement(ControlFlowGraph graph) {
    ...
    for (BasicBlock block : blocks) {
      ...
      for (BasicBlock succex : block.getSuccExceptions()) {
        ...
        if (range != null && !range.isCircular()) {
...
//        if (!mapSortOrder.containsKey(stt.id)) {
//          continue;
//        }
      ...
      lstPosts.sort(Comparator.comparing(key -> {
        final Integer order = mapSortOrder.get(key);
        if (order == null) {
          return 0;
        }
  ...
  public static RootStatement parseGraph(ControlFlowGraph graph) {
    ...
    try {
      DotExporter.toDotFile(root, new File("c:\\Temp\\root_before.dot"));
    } catch (Exception ex) {
      ex.printStackTrace();
    }
    if (!processStatement(root, new HashMap<>())) {
      ...
      try {
        DotExporter.toDotFile(root, new File("c:\\Temp\\root.dot"));
      } catch (Exception ex) {
        ex.printStackTrace();
      }
      //			try {
    ...
    try {
      DotExporter.toDotFile(root, new File("c:\\Temp\\root_after.dot"));
    } catch (Exception ex) {
      ex.printStackTrace();
    }
    return root;
  ...
  private static boolean processStatement(Statement general, HashMap<Integer, Set<Integer>> mapExtPost) {
    ...
    for (int mapstage = 0; mapstage <= 2; mapstage++) {
      ...
      for (int reducibility = 0;
        ...
        try {
          DotExporter.toDotFile(general, new File("c:\\Temp\\processStatement_stat1.dot"));
        } catch (Exception ex) {
          ex.printStackTrace();
        }
      }
