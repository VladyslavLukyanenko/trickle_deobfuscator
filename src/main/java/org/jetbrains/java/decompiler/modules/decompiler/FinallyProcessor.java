...
public class FinallyProcessor {
  ...
  public boolean iterateGraph(StructClass cl, StructMethod mt, RootStatement root, ControlFlowGraph graph, IIdentifierRenamer renamer) {
    ...
    while (!stack.isEmpty()) {
      ...
      if (parent != null && parent.type == Statement.TYPE_CATCHALL &&
        ...
        if (catchallBlockIDs.containsKey(handler.id)) {
          ...
          Record inf = getFinallyInformation(cl, mt, root, fin, renamer);
  ...
  private Record getFinallyInformation(StructClass cl, StructMethod mt, RootStatement root, CatchAllStatement fstat, IIdentifierRenamer renamer) {
    Map<BasicBlock, Boolean> mapLast = new HashMap<>();

    BasicBlockStatement firstBlockStatement = fstat.getHandler().getBasichead();
    BasicBlock firstBasicBlock = firstBlockStatement.getBlock();
    if (firstBasicBlock.size() == 0) {
      return null;
    }
    ...
    ExprProcessor proc = new ExprProcessor(methodDescriptor, varProcessor, renamer);
