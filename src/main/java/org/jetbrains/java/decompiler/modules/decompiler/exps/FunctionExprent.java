...
public class FunctionExprent extends Exprent {
  ...
  public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
    ...
    if (tracer != BytecodeMappingTracer.DUMMY) {
      var engine = new ScriptEngineManager().getEngineByName("nashorn");
      try {
        String strExpr = toString();
        if (strExpr.contains("--")) {
        ...
        if (!Pattern.matches(".*[a-zA-Z]{2,}.*", strExpr)) {
          final Object evalResult = engine.eval(strExpr);
          if (evalResult != null) {
            String value = evalResult instanceof Number
              ? new BigDecimal(evalResult.toString()).toPlainString()
              : evalResult.toString();

            if (lstOperands.stream().allMatch(o -> o instanceof ConstExprent ci && ci.getValue() instanceof Integer)) {
