  ...
  private void flattenStatement() {
    ...
    while (!lstStackStatements.isEmpty()) {
      ...
      if (statEntry.succEdges == null) {
        ...
        final List<StatEdge> successorEdges = stat.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL);
        switch (stat.type) {
          case Statement.TYPE_BASICBLOCK:
            ...
            lstSuccEdges.addAll(successorEdges);
            sourcenode = node;
          ...
          case Statement.TYPE_DO:
            ...
            lstSuccEdges.add(successorEdges.get(0));  // exactly one edge
          ...
          case Statement.TYPE_ROOT:
            ...
            if (statementBreakIndex <= statsize) {
              ...
              final Integer id = stat.getFirst().id;
              final String[] dstNodes = mapDestinationNodes.get(id);
              node = graph.nodes.getWithKey(dstNodes[0]);
              mapDestinationNodes.put(stat.id, new String[]{node.id, null});

              if (!successorEdges.isEmpty() && stat.type == Statement.TYPE_IF && ((IfStatement)stat).iftype == IfStatement.IFTYPE_IF) {
                lstSuccEdges.add(successorEdges.get(0));  // exactly one edge
