...
public class VarDefinitionHelper {
    ...
    while (!stack.isEmpty()) {
      ...
      if (lstVars != null) {
        for (VarExprent var : lstVars) {
          ...
          varproc.setVarName(new VarVersionPair(var), vc.getFreeName(var.getIndex(), var.getVarType()));
  ...
  public void setVarDefinitions() {
    ...
    for (Entry<Integer, Statement> en : mapVarDefStatements.entrySet()) {
      ...
      final var versionPair = new VarVersionPair(index.intValue(), 0);
      var varType = varproc.getVarType(versionPair);
      varproc.setVarName(versionPair, vc.getFreeName(index, varType));
