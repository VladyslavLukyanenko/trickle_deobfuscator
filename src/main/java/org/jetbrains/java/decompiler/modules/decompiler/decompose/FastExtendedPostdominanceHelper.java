  ...
  private void removeErroneousNodes() {
    ...
    iterateReachability((node, mapSets) -> {
      ...
      for (StatEdge prededge : node.getPredecessorEdges(StatEdge.TYPE_REGULAR)) {
        final Integer sourceId = prededge.getSource().id;
        FastFixedSet<Integer> setPred = mapSets.get(sourceId);
        if (setPred == null) {
//          if (!mapSupportPoints.containsKey(sourceId)) {
//            continue;
//          }

          setPred = mapSupportPoints.get(sourceId);
        }
      ...
      for (Integer id : setReachability) {
        ...
        for (FastFixedSet<Integer> predset : lstPredSets) {
          if (predset != null && predset.contains(id)) {
