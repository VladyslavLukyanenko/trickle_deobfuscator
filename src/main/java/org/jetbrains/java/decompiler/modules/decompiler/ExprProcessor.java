...
public class ExprProcessor implements CodeConstants {
  public static final String UNDEFINED_TYPE_STRING = "__UNDEFINED__";
  public static final String UNKNOWN_TYPE_STRING = "__UNKNOWN__";
  public static final String NULL_TYPE_STRING = "__NULL__";
  ...
  public ExprProcessor(MethodDescriptor md, VarProcessor varProc, IIdentifierRenamer renamer) {
    methodDescriptor = md;
      ...
      switch (instr.opcode) {
        ...
        case opc_invokedynamic:
          if (instr.opcode != opc_invokedynamic || instr.bytecodeVersion >= CodeConstants.BYTECODE_JAVA_7) {
            ...
            final String oldClassName = invoke_constant.classname;
            var className = oldClassName != null ? oldClassName.replace(".", "/") : null;
            var methodName = invoke_constant.elementname;
            var descriptor = invoke_constant.descriptor;
            var ix1 = invoke_constant.index1;
            var ix2 = invoke_constant.index2;

            var simpleClassName = className != null && className.contains("/")
              ? className.substring(className.lastIndexOf('/') + 1)
              : className;
            if (className != null && renamer != null && renamer.toBeRenamed(IIdentifierRenamer.Type.ELEMENT_CLASS, simpleClassName, "", "")) {
              className = className.substring(0, className.lastIndexOf('/') + 1) + renamer.getNextClassName(className, simpleClassName);
              className = className.replace("/", ".");
            }

            if (className != null && renamer != null && renamer.toBeRenamed(IIdentifierRenamer.Type.ELEMENT_METHOD, className, methodName, descriptor)) {
              methodName = renamer.getNextMethodName(oldClassName, methodName, descriptor);
            }

            InvocationExprent exprinv = new InvocationExprent(instr.opcode, className, methodName, descriptor, ix1, ix2,
  ...
  public static String getTypeName(VarType type, boolean getShort) {
    var nonNormalizedName = getTypeNameWithoutNormalization(type, getShort);

    return MemberNameUtil.ensureClsNameIsSimple(nonNormalizedName);
  }

  private static String getTypeNameWithoutNormalization(VarType type, boolean getShort) {
    int tp = type.type;
    if (tp <= CodeConstants.TYPE_BOOLEAN) {
      ...
      if (getShort) {
        final ImportCollector importCollector = DecompilerContext.getImportCollector();
        if (importCollector == null) {
          if (ret.contains(".")) {
        ...
        ret = importCollector.getShortName(ret);
      }
