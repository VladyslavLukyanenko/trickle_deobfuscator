...
import org.jetbrains.java.decompiler.main.decompiler.StrSourceBytesCollector;
  ...
  public InvocationExprent(int opcode,
    ...
    switch (opcode) {
      ...
      case CodeConstants.opc_invokedynamic:
        ...
        this.classname = "java/lang/Class"; // dummy class name
        invokeDynamicClassSuffix = "##Lambda_" + index1 + "_" + index2;
    }
    ...
    stringDescriptor = methodDescriptor;
...
//
//    if (classname.equals("Adidas/b")
//      && name.equals("indexOf")
//      && lstParameters.size() == 2
//      && lstParameters.get(0) instanceof ConstExprent deobfParam
//      && lstParameters.get(1) instanceof ConstExprent strParam) {
//      var bytes = StrSourceBytesCollector.get(deobfParam.getValue().toString());
//      buf.append(b.indexOf(strParam.getIntValue(), new String(bytes)));
//      return buf.enclose("'", "'");
//    }
      ...
      case TYP_INIT:
        if (super_qualifier != null) {
          ...
          buf.append(instance.toJava(indent, tracer))
            .append(" = new ")
            .append(DecompilerContext.getImportCollector().getShortName(classname.replace("/", ".")))
            .append("(");
//          buf.append(instance.toJava(indent, tracer)).append(".<init>(");
    ...
    // todo: remove this hack. its for trickle only
    final String str = buf.toString();
    if (str.startsWith("while__") || str.startsWith("fuck__") || str.startsWith("super(")) {
      buf.prepend("// ");
    }
