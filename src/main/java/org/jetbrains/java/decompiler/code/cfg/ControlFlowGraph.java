...
public class ControlFlowGraph implements CodeConstants {
  ...
  private void eliminateFantomBlocks() {
    if (blocks.size() < 2) {
      return;
    }

    var fantomBlocks = new ArrayList<BasicBlock>();
    for (int i = 1; i < this.blocks.size(); i++) {
      BasicBlock b = this.blocks.get(i);

      boolean includedInFlow = false;
      for (var candidate : this.blocks) {
        if (b == candidate) {
          continue;
        }

        includedInFlow = candidate.getSuccs().contains(b);
        if (includedInFlow) {
          break;
        }
      }
    ...
    for (var fantom : fantomBlocks) {
      removeBlock(fantom);
//      this.blocks.removeWithKey(fantom.id);
    }
  }
    ...
    for (ExceptionHandler handler : instrseq.getExceptionTable().getHandlers()) {
      ...
      if (mapRanges.containsKey(key)) {
        ...
        for (int j = from.id; j < to.id; j++) {
          ...
          if (block == null) {
            continue; // maybe it was fantom block and it eliminated
          }
