...
public class DotExporter {
  ...
  private static final boolean DUMP_DOTS = true/* DOTS_FOLDER != null*/;
  ...
  private static File getFile(StructMethod mt, String suffix) {
    File root = new File(DOTS_FOLDER + mt.getName());
    if (!root.isDirectory())
      root.mkdirs();
  ...
  public static void toDotFile(Statement stat, File file) {
    if (!DUMP_DOTS)
      return;
    try{
  ...
  public static void toDotFile(VarVersionsGraph graph, File file) {
    if (!DUMP_DOTS)
      return;
    try{
      BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(file));
  ...
  public static void toDotFile(ControlFlowGraph graph, File file, boolean showMultipleEdges) {
    if (!DUMP_DOTS)
      return;
    try{
      BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(file));
