...
public class StructMethod extends StructMember {
  ...
  private InstructionSequence parseBytecode(DataInputFullStream in, int length, ConstantPool pool) throws IOException {
    ...
    for (int i = 0; i < exception_count; i++) {
      ...
      final int from = Math.min(maxOffset, Math.max(0, in.readUnsignedShort()));
      final int to = Math.min(maxOffset, in.readUnsignedShort());
      handler.from = Math.min(from, to);
      handler.to = Math.max(from, to);
