...
public class ConstantPool implements NewClassNameBuilder {
  ...
  public ConstantPool(DataInputFullStream in) throws IOException {
    ...
    for (int i = 1; i < size; i++) {
      ...
      switch (tag) {
        case CodeConstants.CONSTANT_Utf8:
          var offset = in.getPos();
          final var utf = in.readUTF();
          var raw = in.readAtOffset(offset, in.getPos() - offset);
          StrSourceBytesCollector.put(utf, raw);
          pool.add(new PrimitiveConstant(CodeConstants.CONSTANT_Utf8, utf));
          break;
